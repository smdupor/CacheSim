...
void Cache::L1_stats_report() {
   ...
   cat_padded(&output, (double)std::round(10000*((double)this->vc_swap_requests)/((double)this->reads+(double)this->writes))/10000);
   output += "  g. number of swaps:                   ";
   cat_padded(&output, this->vc_swaps);
   output += "  h. combined L1+VC miss rate:          ";
   cat_padded(&output, (double)std::round(10000*((double)(this->read_misses+this->write_misses-this->vc_swaps))/((double)(this->reads+this->writes)))/10000);
...
void Cache::L2_stats_report() {
   ...
   if (level == L2) { // This hierarchy has a level-2 cache
      ...
      cat_padded(&output, (double)std::round(10000*(double)this->read_misses / (double)this->reads)/10000);
