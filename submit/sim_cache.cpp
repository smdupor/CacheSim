/**
 * Cache Simulator Main Function. Submission from STEVAN DUPOR, smdupor, for NCSU ECE563 Fall 2021.
 *
 * Based on template file with refactoring.
 */
...
/**
 * Print the data about the contents of the cache configuration. Use Cache class's padded printing function to match
 * whitespace padding from validation traces.
 *
 * @param trace_file path to the memory trace file (for printing)
 * @param params the physical parameters of this cache hierarchy
 */
void print_parameters_block(const char *trace_file, const cache_params &params) {
